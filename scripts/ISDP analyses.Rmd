...
QC <- read.csv(here("data","HAPPE_pipelineQC_Y1.csv"), header = T)
...
pren_long %>%
  ...
  ylab("Caregiver Perceived Stress")+
...
pren_long %>%
  ...
  theme_minimal()+
  theme(legend.position = "none")+
  ylab("Caregiver Depression (EPD Scores)")+
  theme(axis.title = element_text(size = 14),
        legend.title = element_text(size = 12),
        legend.text = element_text(size = 12))
```
...
ggplot(data = pren_long_nao, aes(x = time, y = pred_quad)) +
  ...
  geom_line(aes(x = time, y = dep, group = id, color=), color="#c6880a", alpha=.35) +
...
write.table(pren_stress, here("data", "stress_preds.csv"),sep = ",", row.names = F)
```
...
ggplot(all_sync_exc, aes(x = trial, y = FrontalSync, fill = trial)) +
  ...
  geom_signif(comparisons = list(c("Odd","After"), c("Std","Odd")), 
             map_signif_level=c("*"=0.2))+
  scale_x_discrete(limits=c("Std", "Odd", "After"), labels=c("Standard", "Deviant", "After-Deviant"))+
  scale_fill_viridis_d( option = "C", end = .75)+
  annotate(geom="text", x=2, y=.35, label="(B = -0.019, p = .012)",
              color="black")+
...
ggplot(all_sync_exc, aes(x = trial, y = CentralSync, fill = trial)) +
  ...
  annotate(geom="text", x=3, y=.65, label="(B = -0.016, p = .017)",
              color="black")+
  annotate(geom="text", x=1, y=.65, label="(B = -0.015, p = .026)",
              color="black")+
...
```{r}
df_all_eeg <- merge(df_all_eeg, pren_stress, by = "id")
...
### Effect of social disadvantage
```{r}
FrontalDis <- lmer(FrontalSync ~ 1 + 
                     child_age_y1_assessment +  child_sex + ga.c + trial +
                     trial*disadv_prenatal + 
                     
              (1 | id), 
            data = df_all_eeg)
summary(FrontalDis)

CentralDis <- lmer(CentralSync ~ 1 + 
                     child_age_y1_assessment +  child_sex + ga.c + trial +
                     trial*disadv_prenatal + 
...
### Models including depression parameters
```{r}
FrontalDep <- lmer(FrontalSync ~ 1 + 
                     child_age_y1_assessment +  child_sex + ga.c + disadv_prenatal + trial +
...
```
### Results: 
Depression Intercept significantly predicted weaker Central coupling during odd trials. This effect emerged in the same direction at Frontal and Parietal regions, but was only margingally significant. 
...
```{r , echo=FALSE}
#Cmodel <- 
interactions::interact_plot(CentralDep, pred = trial, modx = Dep_int, plot.points = T, legend.main = "Depression Int", colors = c("#96695a", "#c6880a", "#3ca277"))
...
table(df_all_eeg$Dep_cat, useNA = "ifany")

df_all_eeg%>%
  filter(!is.na(trial))%>%
  ggplot(aes(x = trial, y = CentralSync, fill = trial))+
  geom_boxplot(alpha=.7)+
  scale_fill_manual(values = c("#96695a", "#c6880a", "#3ca277"))+
  facet_wrap(~ factor(Dep_cat, levels = c("-1SD", "Mean", "+1SD")))+
  ylab("Central Delta-Beta Coupling")+
  xlab("Trial Type")+
  ggtitle("Depression Intercept")+
  theme_minimal()+
  theme(legend.position = "none",
...
FrontalTEMP <- lmer(FrontalSync ~ 1 + 
                      ...
                      #child_age_y1_assessment + child_sex + ga.c + 
                      disadv_prenatal + 
                      
                      trial*ibqr_neg.c +
                      trial*Dep_int*ibqr_neg.c + 
              (1 | id), 
            data = df_all_eeg)
summary(FrontalTEMP)
...
suppressWarnings(
  print(
df_all_eeg%>%
  select(id, ibqr_neg.c, Dep_int, Dep_Slope, Dep_QSlope, stress_ave, disadv_prenatal)%>%
  unique()%>%
  ggpairs(., 
        columns = 2:6, 
